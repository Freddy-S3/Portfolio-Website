...
if (-not $xelatex) {
    $fallback = "$env:LOCALAPPDATA\Programs\MiKTeX\miktex\bin\x64\xelatex.exe"
    if (Test-Path $fallback) {
        $xelatex = $fallback
    }
    else {
        Write-Error "xelatex not found. Install MiKTeX with: winget install MiKTeX.MiKTeX"
    }
}
...
try {
    ...
    foreach ($pass in 1, 2) {
        ...
        # Start-Process keeps xelatex's stderr from being wrapped in NativeCommandError.
